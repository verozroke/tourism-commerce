  ...
  <div class="absolute inset-0 py-28">
    <div class="container py-10  flex flex-col gap-10 w-full px-0">
      <div class="flex p-6 gap-3 items-center rounded-[20px]" style="border: 1px solid #E9ECEF">
        <span class="text-oxford flex-grow leading-5 text-lg font-medium">Showing 1 - 10 of 2037 results for
          “cultural”</span>
        <span class="text-slate-500 leading-5">Sort by:</span>
        <div class="w-[150px]">
          <v-select density="compact" :hide-details="true" color="#28B0A6" @update:menu="() => {
      ...
      <div class="flex gap-20">
        <div class="flex flex-col w-[424px] h-fit py-8 px-12 bg-white rounded-2xl" style="border: 1px solid #E9ECEF">
          ...
          <div class="relative h-12 rounded-md w-full mb-8">
            <input @change="() => tourStore.getTourInfosWithDebounce()" v-model="tourStore.fromDate" type="date"
              class="w-full px-4 h-full" style="border-radius: 8px; border: 1px solid #DCE5EB;">
          </div>
          <div class="text-oxford -mt-4 tracking-wide mb-2">Until:</div>
          <div class="relative h-12 rounded-md w-full mb-8">
            <input @change="() => tourStore.getTourInfosWithDebounce()" v-model="tourStore.toDate" type="date"
              class="w-full px-4 h-full" style="border-radius: 8px; border: 1px solid #DCE5EB;">
          </div>
        ...
        <div class="flex flex-col items-center gap-5">
          <div class="grid flex-grow h-fit grid-cols-2 gap-y-8 gap-x-8">
          ...
          <v-pagination color="#28B0A6" v-model="page" :length="4" rounded="circle"></v-pagination>
...
import { SpecialOption, rangeValue } from '~/types/tour';
